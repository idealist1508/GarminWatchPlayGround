...
namespace vestervang.DotNetBlueZ
{
    public static class BlueZManager
    {
        ...
        private static readonly object ProxyCacheLocker = new();
        ...
        {
            var adapters = GetProxies<IAdapter1>(BluezConstants.AdapterInterface, rootObject: null);
        ...
        internal static  IReadOnlyList<T> GetProxies<T>(string interfaceName, IDBusObject rootObject)
        {
            //Upon first run do make sure the proxy is set in the initial cache
            lock (ProxyCacheLocker)
            {
                // TODO: Find a better way of doing this 
                _proxyCache ??= RebuildProxyCache().Result; 
            }
